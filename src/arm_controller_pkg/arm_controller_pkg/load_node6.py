...
class LoadNode(Node):
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().info('[LOAD] data channel connected')
        except Exception as e:
            self.robot_data = None
            self.get_logger().warn(f'[LOAD] data channel connect failed: {e}')
    ...
    def is_at_home(self, tol_deg=1.0):
        ...
        try:
            ...
            self.get_logger().warn(f'[LOAD] is_at_home read failed: {e}')
            return False
    ...
    def go_home(self):
        ...
        if self._at_home or self.is_at_home():
            self._at_home = True
            self.get_logger().info('[LOAD] already at home, skip go_home')
            return True
